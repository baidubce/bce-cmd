...
{{define "FormatCommandsOneLevel"}}\
{{range .FlattenedCommandsOneLevel}}\
{{if not .Hidden}}\
  {{.FullCommand}}
{{.Help|Wrap 4}}\
{{end}}\
...
usage: {{.App.Name}} {{.Context.SelectedCommand}}{{template "FormatUsage" .Context.SelectedCommand}}
{{else}}\
usage: {{.App.Name}}{{template "FormatCommand" .App}}
{{end}}
...
Subcommands:
{{template "FormatCommandsOneLevel" .Context.SelectedCommand}}
{{end}}\
{{else if .App.Commands}}\
Commands:
{{template "FormatCommandsOneLevel" .App}}
{{end}}\
`
