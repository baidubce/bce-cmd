...
func New(name, help string) *Application {
	a := &Application{
		...
		usageTemplate: SeparateOptionalFlagsUsageTemplate,
		terminate:     os.Exit,
	}
...
// on.
func (a *Application) Parse(args []string) (command string, err error, context *ParseContext) {
	...
	if a.completion {
		...
		a.maybeHelp(context)
		if !context.EOL() {
			return "", fmt.Errorf("unexpected argument '%s'", context.Peek()), context
		}

		if setValuesErr != nil {
			//a.writeUsage(context, nil)
			return "", setValuesErr, context
		}
